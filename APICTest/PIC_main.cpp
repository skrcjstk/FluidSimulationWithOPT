...
void timeStep()
{
	...
	timer3.start();
	//pic.GetAPICDescriptorAll(dataForPICDescriptor, desc_width);
...
void render()
{
	...
	for (int k = 0; k<nijk[2]; k++)
		for (int j = 0; j<nijk[1]; j++)
			...
			{
				Vector3f pos = pic.GetGridPos(i, j, k);

				float m_value = pic.GetMass(pos);
				if(m_value != 0)
					boxPrimi.renderPoint(pos, boxColor, 1.0f);

				Vector3f end = (pos + 0.1f * pic.GetVelocity(pos));
...
void buildModel()
{
	...
	pic.Initialize(world, containerStart, containerEnd - containerStart, Vector3i((int)(bSize[0] * 10), (int)(bSize[1] * 10), (int)(bSize[2] * 10)), 1.0);
