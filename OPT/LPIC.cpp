...
void LPIC::Initialize(Vector3f p_bSize, Vector3i p_nCount, float p_rho)
{
	rho = p_rho;
...
void LPIC::LAPICDesc(float result[], FParticle* p_center, std::vector<FParticle*>& p_list, float p_radii)
{
	// Assign Cells
			...
			for (int j = 0; j < nj; j++)
				for (int i = 0; i < ni + 1; i++)
				{
					...
					for (FParticle* p : neighbors)
					{
						if (p->m_pid == Fluid)
						{
							float weight = 4.0f / 3.0f * M_PI * rho * p_radii * p_radii * p_radii * linear_kernel(p->m_curPosition - pos, dx);
			...
			for (int j = 0; j < nj + 1; j++)
				for (int i = 0; i < ni; i++)
				{
					...
					for (FParticle* p : neighbors)
					{
						if (p->m_pid == Fluid)
						{
							float weight = 4.0f / 3.0f * M_PI * rho * p_radii * p_radii * p_radii * linear_kernel(p->m_curPosition - pos, dy);
...
#pragma omp for schedule(static)
		for (int k = 0; k < nk + 1; k++)
			for (int j = 0; j < nj; j++)
				for (int i = 0; i < ni; i++)
				{
					Vector3f pos = Vector3f((i + 0.5f)*dx, (j + 0.5f)*dy, k * dz) + origin;
					std::vector<FParticle *> neighbors;
...
void LPIC::GetNeigboringParticles_cell(int i, int j, int k, int wl, int wh, int hl, int hh, int dl, int dh, std::vector<FParticle *>& res)
{
...
Vector3f LPIC::GetVelocity(Vector3f& pos)
{
