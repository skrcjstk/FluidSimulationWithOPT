...
{
	sscanf_s((const char*)glGetString(GL_VERSION), "%d.%d", &major_version, &minor_version);
}
...
void MiniGL::initTweakBar()
{
	...
	TwAddVarCB(m_tweakBar, "Camera Rotation", TW_TYPE_QUAT4F, setRotationCB, getRotationCB, &m_quat, 
		" label='Camera Rotation' open help='Change the rotation.' ");

	TwAddVarCB(m_tweakBar, "Camera Zoom", TW_TYPE_FLOAT, setZoomCB, getZoomCB, &m_zoom, " label='Camera Zoom' help='Change the zoom.' min=1.0 max = 10.0 step=0.1 ");
...
void TW_CALL MiniGL::setZoomCB(const void *value, void *clientData)
{
	const float val = *(const float *)(value);
	m_zoom = val;
}

void TW_CALL MiniGL::getZoomCB(void *value, void *clientData)
{
	*(float *)(value) = m_zoom;
}
